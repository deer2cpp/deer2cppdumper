...
namespace Il2CppDumper
{
    public class StructGenerator
    {
        ...
        public void WriteScript(string outputDir)
        {
            ...
            {
                // Map the generic class data from the il2CppType
                var genericClass = il2Cpp.MapVATR<Il2CppGenericClass>(il2CppType.data.generic_class);

                // Now, you need to manually extract the type definition from the generic class
                var genericTypeDefIndex = genericClass.typeDefinitionIndex; // Assuming it has a reference to the type definition

                // Retrieve the type definition directly (assuming you have access to the metadata)
                var typeDef = metadata.typeDefs[genericTypeDefIndex];  // Get the type definition from metadata

                // Now you can proceed with any necessary processing using 'typeDef'
                ...
                // For typeDef, retrieve the type name directly from metadata
                // Retrieve the type name for typeDef directly from metadata
                // For typeDef, retrieve the type name directly from metadata
                var typeToReplaceName = FixName(metadata.GetStringFromIndex((uint)typeDef.nameIndex));

                // For il2CppType, assuming we can map it to a type definition or class:
                var typeReplaceName = FixName(metadata.GetStringFromIndex((uint)il2CppType.data.klassIndex)); // Assuming classIndex or similar exists
            ...
            foreach (var imageDef in metadata.imageDefs)
            {
                ...
                for (int typeIndex = imageDef.typeStart; typeIndex < typeEnd; typeIndex++)
                {
                    var typeDef = metadata.typeDefs[typeIndex];
                    AddStruct(typeDef);
                    var typeName = metadata.GetStringFromIndex((uint)typeDef.nameIndex);
                        ...
                        if (il2Cpp.methodDefinitionMethodSpecs.TryGetValue(i, out var methodSpecs))
                        {
                            foreach (var methodSpec in methodSpecs)
                            {
                                ...
                                if (genericMethodPointer > 0)
                                {
                                    ...
                                    // Assuming methodSpec has a field that references the method definition index
                                    var methodDefinition = metadata.methodDefs[methodSpec.methodDefinitionIndex]; // Example: accessing method definition

                                    // Get the method's name and type
                                    var methodSpecTypeName = metadata.GetStringFromIndex((uint)methodDefinition.customAttributeIndex); // Get type name from metadata
                                    var methodSpecMethodName = metadata.GetStringFromIndex((uint)methodDefinition.nameIndex); // Get method name from metadata

                                    // Combine to create the full method name
                                    var methodFullName = methodSpecTypeName + "$$" + methodSpecMethodName;
                                    scriptMethod.Name = methodFullName;

                                    // Combine the type name and method name to create the full method name
                                    var methodFull = methodSpecTypeName + "$$" + methodSpecMethodName;

                                    // Set the script method name
                                    scriptMethod.Name = methodFull;


                                    var methodReturnType = il2Cpp.types[methodDef.returnType];
                                    // Assuming 'methodReturnType' is a reference to the return type of the method (could be an index or a type object)
                                    var returnType = ParseType(methodReturnType, null);  // Parse the return type using the generic context (if applicable)
                                    ...
                                    for (var j = 0; j < methodDef.parameterCount; j++)
                                    {
                                        ...
                                        var parameterName = metadata.GetStringFromIndex(parameterDef.nameIndex);
                                        var parameterType = il2Cpp.types[parameterDef.typeIndex];
                                        var parameterCType = ParseType(parameterType, null);
                                        if (parameterType.byref == 1)
                                        {
                                            parameterCType += "*";
                                        }
            ...
            {
                var sectionHelper = executor.GetType();
                var sec = default(Type);
                Console.WriteLine(sectionHelper.GetType());  // This will tell you what type sectionHelper is.
                {
                    // Assuming 'sec' is an object with 'GenericParameterPosition' and 'Namespace' properties
                    il2Cpp.Position = il2Cpp.Position;

                    // Ensure sec.Namespace and il2Cpp.Length are valid for comparison
                    var end = il2Cpp.PointerSize;

                    while (sec.MetadataToken < (int)end)
                    {
        ...
        private void AddMetadataUsageFieldInfo(ScriptJson json, uint index, ulong address)
        {
            ...
            var typeDef = (type);
            var fieldDef = metadata.fieldDefs[fieldRef.fieldIndex];
            var fieldName = executor.GetType() + "." + metadata.GetStringFromIndex(fieldDef.nameIndex);
            var scriptMetadata = new ScriptMetadata();
            json.ScriptMetadata.Add(scriptMetadata);
            scriptMetadata.Address = il2Cpp.GetRVA(address);
        ...
        private void AddMetadataUsageMethodRef(ScriptJson json, uint index, ulong address)
        {
            ...
            json.ScriptMetadataMethod.Add(scriptMetadataMethod);
            scriptMetadataMethod.Address = il2Cpp.GetRVA(address);
            var result = executor.GetType();
            var methodSpecTypeName = result; // Assuming TypeName is a property of the result
            var methodSpecMethodName = result; // Assuming MethodName is a property of the result
        ...
        private void AddGenericClassStruct(ulong pointer)
        {
            ...
            var typeDef = executor.GetTypeDefinitionFromIl2CppType;
            var structInfo = new StructInfo();
            structInfoList.Add(structInfo);
            structInfo.TypeName = genericClassStructNameDic[pointer];
            // Fix the assignment based on the type of typeDef
            // Check if the type is a value type (struct or enum)
            // Check if the type is a value type (struct or enum)
        ...
        {
            var imageName = typeDefImageNames[typeDef];
            var collection = executor.GetTypeDefinitionFromIl2CppType; // Assuming this returns a collection of target definitions

            if (collection != null)
            {
                // Iterate through each definitionData in the collection

                {
                    ...
                    {
                        type = Il2CppRGCTXDataType.IL2CPP_RGCTX_DATA_TYPE, // Set the type (could be TYPE, CLASS, METHOD, etc.)
                        data = new Il2CppRGCTXDefinitionData
                        {
                            
                        },
                        typeIndex = 1,   // Set the type index for the DefinitionData
                        methodIndex = -1 // Set the method index for the DefinitionData
                    };
                    ...
                    {
                        // Directly use definitionData.data for mapping in both versions
                        rgctxDefData = definitionData.data;
                    }
                    else
                    {
                        // Use definitionData.data for older versions as well
                        rgctxDefData = definitionData.data;
                    }

                    

                    // Switch case to handle different types of RGCTX data
                    switch (definitionData.type)
                    {
                        case Il2CppRGCTXDataType.IL2CPP_RGCTX_DATA_TYPE:
                            {
                                var il2CppType = il2Cpp.types[rgctxDefData.typeIndex]; // Access type by index
                                Console.WriteLine(il2CppType); // Check the full object structure to see its properties
                                break;
                        ...
                        case Il2CppRGCTXDataType.IL2CPP_RGCTX_DATA_CLASS:
                            {
                                var il2CppType = il2Cpp.types[rgctxDefData.typeIndex]; // Access class by index
                                Console.WriteLine(il2CppType); // Assuming Name is the property for class name
                                break;
                            }
                        case Il2CppRGCTXDataType.IL2CPP_RGCTX_DATA_METHOD:
                            {
                                var methodSpec = il2Cpp.methodSpecs[rgctxDefData.methodIndex]; // Access method by index
                                Console.WriteLine("MethodSpec: " + methodSpec); // Inspect the methodSpec object

                                // Assuming methodSpec has a direct reference to the type and method name:
                                var methodType = methodSpec; // This assumes we still need to get the method type by index
                                Console.WriteLine("MethodType: " + methodType); // Inspect the method type object

                                // Directly access the method name or other properties of methodSpec
                                var methodName = methodSpec; // If methodSpec directly holds the name, use it
                                Console.WriteLine("MethodName: " + methodName); // Inspect the method name

                                // Construct the full method name
                                structRGCTXInfo.MethodName = methodType + "." + methodName; // Construct the full method name
                                Console.WriteLine("Full MethodName: " + structRGCTXInfo.MethodName); // Inspect the full method name
        ...
        private List<StructRGCTXInfo> GenerateRGCTX(string imageName, Il2CppMethodDefinition methodDef)
        {
            ...
            var collection = executor.GetTypeDefinitionFromIl2CppType;
            if (collection != null)
            {
                
                {
                    ...
                    if (il2Cpp.Version >= 27.2)
                    {
                        var definitionDataPointer = new DefinitionData(); // Replace '_data' with the actual field if needed
        ...
        private Il2CppTypeDefinition GetIl2CppTypeDefinition(Il2CppType il2CppType) // Updated to take Il2CppType as parameter
        {
            switch (il2CppType.type)
            {
                case Il2CppTypeEnum.IL2CPP_TYPE_VOID:
                ...
                case Il2CppTypeEnum.IL2CPP_TYPE_GENERICINST:
                    // Handle generic types (e.g., List<int>, Dictionary<TKey, TValue>)
                    var genericClass = il2Cpp.MapVATR<Il2CppGenericClass>(il2CppType.data.generic_class);
                    // Further processing of the generic class if needed
                    return executor.GetTypeDefinitionFromIl2CppType(null);
        ...
        {
            var typeName = executor.GetType();
            var typeStructName = executor.GetTypeDefinitionFromIl2CppType;
            var uniqueName = GetUniqueName(null);
        ...
        private bool IsCustomType(Il2CppType il2CppType, Il2CppGenericContext context)
        {
            switch (il2CppType.type)
            {
                ...
                case Il2CppTypeEnum.IL2CPP_TYPE_GENERICINST:
                    {
                        var genericClass = il2Cpp.MapVATR<Il2CppGenericClass>(il2CppType.data.generic_class);
                        var typeDef = executor.GetTypeDefinitionFromIl2CppType(null);
                        if (typeDef.IsEnum)
                        {
                            return IsCustomType(il2Cpp.types[typeDef.elementTypeIndex], context);
                        }
    ...
    {
        public Il2CppRGCTXDataType type { get; set; }
        public Il2CppRGCTXDefinitionData data { get; set; }
